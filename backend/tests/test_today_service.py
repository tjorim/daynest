...
class StubTodayRepository:
    def __init__(self, tasks: list[SimpleNamespace], overdue: list[SimpleNamespace], due: list[SimpleNamespace], upcoming: list[SimpleNamespace]):
        self._tasks = tasks
...
def test_get_today_shapes_chore_sections() -> None:
    ...
    repo = StubTodayRepository(tasks=routine_tasks, overdue=overdue, due=due, upcoming=upcoming)
