...
class TestRotateIntegrationClient:
    ...
    def test_rotate_revoked_client_returns_conflict(self, client: TestClient, db_session: Session) -> None:
...
class TestRevokeIntegrationClient:
    def test_revoke_preserves_lifecycle_record(self, client: TestClient, db_session: Session) -> None:
...
class TestExchangeIntegrationClientToken:
    def test_previous_hash_secret_is_upgraded_on_use(
        ...
        monkeypatch.setattr(settings, "_cached_integration_key_hash_secret", "current-secret")
        monkeypatch.setattr(settings, "integration_key_hash_secret_previous", "previous-secret")
        record = IntegrationClient(
            user_id=user.id,
            name="Old secret",
            key_hash=integration_auth._hash_integration_key_with_secret(raw_key, "previous-secret"),
            rate_limit_per_minute=60,
        )
        db_session.add(record)
        db_session.commit()

        authenticated = integration_auth.get_integration_client_by_raw_key(db_session, raw_key)
