...
| Calendar token revoke, session revoke, and resource deletes | MCP tools where exposed, integrations UI | Delete-to-absent. A retry can receive `404`/`204`; clients treat both as an absent final state. Token regeneration/rotation is deliberately excluded because it creates a new secret. |
| Integration-client revoke | MCP `revoke_integration_client`, integrations UI | Ends in the same revoked state, but **is not idempotent**: each repeat rewrites `revoked_at` and appends another audit entry, so MCP advertises `idempotentHint: false` and clients should not retry it automatically. |
...
| Push notification delivery | A durable five-minute pre-send lease prevents overlapping dispatchers. After a provider success, `notification_sent` is recorded and the lease is cleared atomically. An expired lease from an interrupted dispatcher is retried on a later scheduler pass. | **High risk:** this explicitly chooses at-least-once delivery for interrupted/uncertain sends, so a provider call that succeeded just before termination can be duplicated. |
