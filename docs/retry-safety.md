...
| Chore instance assign, complete, skip, reschedule | MCP routine tools, Home Assistant and Pebble actions, Android notification actions, REST bulk | The requested state converges, but complete and skip currently rewrite their timestamp and append another audit entry on an exact replay, while competing transitions are last-writer-wins rather than `409`. **Do not automatically retry after an uncertain response**; re-read the instance first. Add same-transition no-op guards and conflicting-transition checks before treating these operations as retry safe. |
