...
| --- | --- | --- |
| Chore instance assign, complete, skip, reschedule; task instance start, complete, skip | MCP routine tools, Home Assistant and Pebble actions, Android notification actions, REST bulk | Resource-state. Repeating the same transition preserves the state. A competing *different* transition is optimistic concurrency and returns `409`; re-read before deciding what to do. |
...
| Push notification delivery | `notification_sent` is the delivery natural key. | Natural-key deduplication must be claimed before sending. On an uncertain provider response, do not send again unless the provider supplies its own idempotency contract. |
