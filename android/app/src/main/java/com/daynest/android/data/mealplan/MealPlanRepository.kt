...
constructor(
    ...
    suspend fun listMealPlans(): Result<List<MealPlanDto>> = safeApiCall { mealPlanApi.listMealPlans() }
    ...
    suspend fun updateSlot(mealPlanId: Int, slotId: Int, request: MealSlotUpdateDto): Result<MealSlotDto> =
    ...
    private suspend fun upsertCachedSlot(mealPlanId: Int, slot: MealSlotDto) {
        val cacheKey = weekCacheKey(mealPlanId)
        val cachedWeek =
            cacheEntryDao.get(cacheKey)?.payload?.let { payload ->
                JsonSerializer.config.decodeFromString(WeekGridDto.serializer(), payload)
