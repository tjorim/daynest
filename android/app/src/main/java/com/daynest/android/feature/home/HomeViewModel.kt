...
constructor(
    ...
    private fun choreAction(id: Int, complete: Boolean) {
        viewModelScope.launch {
    ...
    private fun rescheduleChore(id: Int, scheduledDate: String) {
        viewModelScope.launch {
    ...
    private fun toggleSelection(type: SectionType, id: Int) {
        _uiState.update { current ->
    ...
    private fun selectAll(type: SectionType, ids: List<Int>) {
        _uiState.update { current ->
            if (current is HomeUiState.Content) {
                when (type) {
    ...
    private fun taskAction(id: Int, complete: Boolean) {
        viewModelScope.launch {
    ...
    private fun doseAction(id: Int, take: Boolean) {
        viewModelScope.launch {
    ...
    private fun updatePlanned(item: PlannedTodayItemDto, scope: EditScope = EditScope.THIS) {
        viewModelScope.launch {
    ...
    private fun markPlannedDone(id: Int, isDone: Boolean) {
        val currentPlanned =
            (_uiState.value as? HomeUiState.Content)
                ?.planned
    ...
    private fun deletePlanned(id: Int, scope: DeleteScope = DeleteScope.THIS) {
        viewModelScope.launch {
...
sealed interface HomeUiEvent {
    ...
    data class StartTaskClicked(val taskInstanceId: Int) : HomeUiEvent

    data class SkipTaskClicked(val taskInstanceId: Int) : HomeUiEvent

    data class RescheduleChoreClicked(val choreInstanceId: Int, val scheduledDate: String) : HomeUiEvent

    data class SnoozeChoreClicked(val choreInstanceId: Int) : HomeUiEvent
    ...
    data class UpdatePlannedClicked(val item: PlannedTodayItemDto, val scope: EditScope = EditScope.THIS) : HomeUiEvent

    data class DeletePlannedClicked(val id: Int, val scope: DeleteScope = DeleteScope.THIS) : HomeUiEvent

    data class ToggleSelection(val type: SectionType, val id: Int) : HomeUiEvent

    data class SelectAll(val type: SectionType, val ids: List<Int>) : HomeUiEvent

    data class ClearSelection(val type: SectionType) : HomeUiEvent

    data class BulkDone(val type: SectionType) : HomeUiEvent

    data class BulkSkip(val type: SectionType) : HomeUiEvent

    data class BulkUndo(val type: SectionType) : HomeUiEvent
}
...
private fun PlannedTodayItemDto.toUpdateDto() = PlannedItemUpdateDto(
    title = title,
