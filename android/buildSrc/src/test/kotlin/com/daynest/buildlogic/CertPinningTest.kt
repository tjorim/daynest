...
class CertPinningTest {
    ...
    fun `resolvePins prefers local properties over Gradle property and env var`() {
        val pins =
            CertPinning.resolvePins(
                key = "prodCertificatePins",
                envKey = "DAYNEST_ANDROID_PROD_CERTIFICATE_PINS",
                localProperty = { key -> "sha256/local".takeIf { key == "prodCertificatePins" } },
    ...
    fun `resolvePins falls back to Gradle property when local properties has no value`() {
        val pins =
            CertPinning.resolvePins(
                ...
                gradleProperty = { key -> "sha256/gradle".takeIf { key == "prodCertificatePins" } },
                env = { "sha256/env" },
            )
    ...
    fun `resolvePins falls back to env var when local properties and Gradle property have no value`() {
        val pins =
            CertPinning.resolvePins(
                ...
                env = { key -> "sha256/env".takeIf { key == "DAYNEST_ANDROID_PROD_CERTIFICATE_PINS" } },
            )
    ...
    fun `requireHostForPins fails when pins are configured but the host is blank`() {
        ...
        assertTrue(exception.message.orEmpty().contains("DAYNEST_ANDROID_PROD_CERTIFICATE_PIN_HOST"))
    }
