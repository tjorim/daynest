...
class DaynestTileService : TileService() {
    ...
    private fun buildTile(snapshot: WearTodaySnapshot?): TileBuilders.Tile = TileBuilders.Tile
        .Builder()
    ...
    private fun buildLayout(snapshot: WearTodaySnapshot?): LayoutElementBuilders.Layout = LayoutElementBuilders.Layout
        .Builder()
        .setRoot(
    ...
    private fun buildClickableModifiers(): ModifiersBuilders.Modifiers = ModifiersBuilders.Modifiers
        .Builder()
        .setClickable(
    ...
    private fun buildLaunchAction(): ActionBuilders.LaunchAction = ActionBuilders.LaunchAction
        .Builder()
        .setAndroidActivity(
    ...
    private fun buildText(text: String): LayoutElementBuilders.Text = LayoutElementBuilders.Text
        .Builder()
        .setText(text)
        .build()
    ...
    private fun getOverdueText(snapshot: WearTodaySnapshot?): String = getString(
        R.string.wear_overdue_count,
        snapshot?.overdueCount ?: 0
    )

    private fun getMedicationText(snapshot: WearTodaySnapshot?): String = snapshot?.nextMedication?.let {
