...
data class WearDueItem(val type: WearDueItemType, val id: Int, val title: String, val subtitle: String? = null)
...
fun TodayResponseDto.toWearTodaySnapshot(): WearTodaySnapshot = WearTodaySnapshot(
...
private fun TodayResponseDto.nextMedicationName(): String? = medication
    .filter { !it.status.isMedicationDoneStatus() }
    .minByOrNull { it.scheduledAt.ifBlank { LAST_SORT_KEY } }
    ?.name

private fun TodayResponseDto.dueItems(): List<WearDueItem> = overdueChoreItems() +
    dueTodayChoreItems() +
    medicationItems()

private fun TodayResponseDto.overdueChoreItems(): List<WearDueItem> = overdue
...
private fun TodayResponseDto.dueTodayChoreItems(): List<WearDueItem> = dueToday
    .filter { !it.status.isDoneStatus() }
    .map { item ->
        WearDueItem(
            type = WearDueItemType.CHORE,
...
private fun TodayResponseDto.medicationItems(): List<WearDueItem> = medication
    .filter { !it.status.isMedicationDoneStatus() }
    .map { item ->
        WearDueItem(
...
private fun String.isMedicationDoneStatus(): Boolean = equals("taken", ignoreCase = true) ||
