...
**Directory:** `custom_components/daynest/[platform]/`
...
from .[entity_file] import Daynest[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: DaynestDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            Daynest[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Daynest."""
...
Add platform to `custom_components/daynest/manifest.json`:
...
- [ ] Entity class inherits from both `DaynestEntity` and platform class
...
- **Domain:** `daynest`
- **Class prefix:** `Daynest`
- **Base entity:** `DaynestEntity` in `entity/base.py`
- **Coordinator:** `DaynestDataUpdateCoordinator`
