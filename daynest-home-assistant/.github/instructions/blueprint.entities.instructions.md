...
- **`entity/`** - Base entity classes (inherit `DaynestEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, DaynestEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import DaynestEntity`
