...
- **Domain:** `daynest`
- **Title:** Daynest
- **Repository:** daynest/daynest-home-assistant

**Key directories:**

- `custom_components/daynest/` - Main integration code
...
- Integration logs: `custom_components.daynest: debug` in `config/configuration.yaml`
...
- Use the domain `daynest` for all DOMAIN references
- Prefix all integration-specific classes with `Daynest`
- Use "Daynest" as the display title
...
- Inherit from platform base + `DaynestEntity`
...
- `tests/` mirrors `custom_components/daynest/` structure
